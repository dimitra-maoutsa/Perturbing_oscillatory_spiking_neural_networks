# -*- coding: utf-8 -*-
"""
Created on Wed Mar 25 02:25:13 2015
...
var_dict3 = {"iter_time1":iter_time1,"iter_time2":iter_time2,"ccf":ccf,"ass":ass,"links":links, "radia":radia, "diameters":diameters,"paths":paths,"ks":ks,"qs":qs,"ns":ns ,"kkk_qqq_nnn":0}
#pickle.dump(var_dict3, open("Links3.dat", "wb"))  
d = pickle.load( open( "Links3.dat", "rb" ) )
iter_time2 = d["iter_time2"]
iter_time1 = d["iter_time1"]
d = pickle.load( open( "Links.dat", "rb" ) )
paths2 = d["paths"]
iter_time3 = d["iter_time"]
#iter_time4 = d["iter_time1"]
cc2 = d["ccf"]

iter_time = np.zeros((size_k,size_q,size_n))
for i in range(len(ks)):
    for j in range(len(qs)):
        for z in range(len(ns)):
            iter_time[i,j,z] = (iter_time1[i,j,z] + iter_time2[i,j,z]+iter_time3[i,j,z] )/3
            
allpaths = np.zeros((size_k,size_q,size_n))
for i in range(len(ks)):
    for j in range(len(qs)):
        for z in range(len(ns)):
            allpaths[i,j,z] = (paths[i,j,z] + paths2[i,j,z])/2
            
            
paths21 = np.zeros((size_k,size_q,size_n))
for i in range(len(ks)):
    for j in range(len(qs)):
        for z in range(len(ns)):
            paths21[i,j,z] = paths2[i,j,z]
...
###### Plot ccf k path
pathq2 =  np.zeros((size_k,size_q,size_n))
pathq1 =  np.zeros((size_k,size_q,size_n))
pathq3 =  np.zeros((size_k,size_q,size_n))
ccfq =  np.zeros((size_k,size_q,size_n))
for i in range(len(qs)): 
    
    #ccfq[:,i,:] = cc2[:,-1-i,:]
    pathq2[:,i,:] = allpaths[:,-1-i,:]
    pathq1[:,i,:] = paths[:,-1-i,:]
    pathq3[:,i,:] = paths21[:,-1-i,:]
    
ccm = np.mean(ccfq,axis = 2)
    
for i in range(6): #ws pros q
    plt.plot(ccfq[i,:,4],'-', label = '$k = %d$' %ks[i])
    #plt.plot(ccm[i,:], label = '$k = %d$' %ks[i])   
    #plt.title('N = 35 x 35')
    plt.xlabel('rewiring probability $q$')
    plt.xticks([0,1,2,3,4,5, 6], ['$0$',r'$2^{-16}$','$2^{-8}$',r'$2^{-4}$','$2^{-2}$','$2^{-1}$', '$1$'])
    plt.ylabel(r'clustering coef. $ CC $')
    #plt.xticks([0,1,2,3,4,5], ['4','8','12','20','24','28'])
...
for i in range(7):
    #plt.figure(1),plt.plot(ccfq[i,:,2],'--')
    plt.plot(itq[i,:,4], label = '$k = %d$' %ks[i])  
    plt.legend(numpoints = 1,loc=1)    
    #plt.title('N = 35 x 35')
    plt.xlabel('rewiring probability $q$')
    plt.xticks([0,1,2,3,4,5, 6], ['$0$',r'$2^{-16}$','$2^{-8}$',r'$2^{-4}$','$2^{-2}$','$2^{-1}$', '$1$'])
...
for i in range(7):
    #plt.figure(1),plt.plot(ccfq[i,:,2],'--')
    if i == 0:
        plt.plot(reltime[:,i,0], label = "$q = 0$") 
    elif i == 6:
        plt.plot(reltime[:,i,0], label = "$q = 1$") 
    else:    
        plt.plot(reltime[:,i,0], label = "$q = 2^{-%d}$" % m[-i])  
    plt.legend(numpoints = 1,loc=1)    
    #plt.title('N = 35 x 35')
    plt.xlabel(' in-degree $k$')
    plt.xticks([0,1,2,3,4,5], ['4','8','12','20','24','28'])
    #plt.xticks([0,1,2,3,4,5, 6], ['$0$',r'$2^{-16}$','$2^{-8}$',r'$2^{-4}$','$2^{-2}$','$2^{-1}$', '$1$'])
    plt.ylabel(r'relative synchr. timescale $ \frac{\tau_{synch}}{\tau_{synch}(k = 4)} $')
...
reltime = np.zeros((size_k,size_q,size_n))
for i in range(len(ks)):
    for j in range(len(qs)):
        for z in range(len(ns)):
            reltime[i,j,z] = itq[i,j,z]/itq[0,j,z]


##### Path length me size
ax = plt.subplot()    
ax.set_yscale("log", nonposy='clip') 
color =  ['bo', 'go' , 'mo', 'ro', 'co','ko', 'yo']
mark = ['.', 's','v','o','*','^','p']
...
for i in range(7):
    ...
    if i == 0:
        ...
        #plt.plot(pathq2[:,i,4],iter_time[:,i,1],'k--')# ,label = '$k = %d$' %ks[i]) 
        plt.plot(allpaths[:,i,4],iter_time[:,i,1],color[i],label = "$q = 1$") 
    elif i == 6:
        
        #plt.plot(pathq2[:,i,4],iter_time[:,i,1],'k--')# ,label = '$k = %d$' %ks[i]) 
        plt.plot(allpaths[:,i,4],iter_time[:,i,1],color[i],label = "$q = 0$")
    else:    
        #plt.plot(reltime[:,i,0], label = "$q = 2^{-%d}$" % m[-i])  
        #plt.plot(pathq2[:,i,4],iter_time[:,i,1],'k--')# ,label = '$k = %d$' %ks[i]) 
        plt.plot(allpaths[:,i,4],iter_time[:,i,1],color[i],label = "$q = 2^{-%d}$" % m[i]) 
    #plt.plot(pathq1[:,i,4],iter_time1[:,i,1],color[i])# ,label = '$k = %d$' %ks[i]) 
    #plt.plot(pathq3[:,i,4],iter_time2[:,i,1],color[i])
...
for j in range(len(ks)):
    for i in range(7):
        ...
        if i == 0:
            ...
            #plt.plot(pathq2[j,i,4],iter_time[:,i,1],'k--')# ,label = '$k = %d$' %ks[i]) 
            plt.plot(diameters[j,i,4],iter_time[j,i,1],color[i]+mark[j],label = "$q = 1$, k = %d" %ks[j]) 
        elif i == 6:
            
            #plt.plot(pathq2[:,i,4],iter_time[:,i,1],'k--')# ,label = '$k = %d$' %ks[i]) 
            plt.plot(diameters[j,i,4],iter_time[j,i,1],color[i]+mark[j],label = "$q = 0$")
        else:    
            #plt.plot(reltime[:,i,0], label = "$q = 2^{-%d}$" % m[-i])  
            #plt.plot(pathq2[:,i,4],iter_time[:,i,1],'k--')# ,label = '$k = %d$' %ks[i]) 
            plt.plot(diameters[j,i,4],iter_time[j,i,1],color[i]+mark[j],label = "$q = 2^{-%d}$" % m[i]) 
        #plt.plot(paths[:,i,4],iter_time1[:,i,1],color[i])# ,label = '$k = %d$' %ks[i]) 
        #plt.plot(paths[:,i,4],iter_time2[:,i,1],color[i])
        plt.legend(numpoints = 1,loc=4)    
        #plt.title('N = 35 x 35')
...
for i in range(len(qs)):
    if i==len(qs)-1:
        ...
        plt.figure(),plt.imshow(iter_time[:,i,:],aspect='auto',cmap='hot')#,
        plt.xticks([0,1,2,3,4,5], ['30x30','35x35','40x40','45x45','50x50','55x55'])
        plt.yticks([0,1,2,3,4,5], ['4','8','12','20','24','28'])
        plt.colorbar()
        plt.title(r'q = $ 0$')
        plt.xlabel('network size n1xn2 ')
        plt.ylabel(' in-degree k ')
...
#itq =  np.zeros((size_k,size_q,size_n))
#for i in range(len(qs)):
#    itq[:,i,:] = iter_time[:,-1-i,:]
...
for i in range(len(ks)):
    if i==0:
        plt.plot(itq[i,:,-1],'ko',label = r'$ N = 55\times 55 $')
        plt.plot(itq[i,:,0],'b^',label = r'$ N = 30\times 30 $')
    plt.plot(itq[i,:,-1],'ko')
    plt.plot(itq[i,:,-1] ,colors1[i],label = '$k = %d$' %ks[i])
    plt.plot(itq[i,:,0],'b^')
    plt.plot(itq[i,:,0] ,colors2[i])
